#include "VJetsUncertaintyTool.h"
...
void VJetsUncertaintyTool::smoothQCDCorrection(bool doSmooth)
{

   m_smoothQCDCorrection = doSmooth;
}

void VJetsUncertaintyTool::mergePDF(bool do_merge_PDF)
{

   m_mergePDF = do_merge_PDF;
}

void VJetsUncertaintyTool::applyDynamicIsolation(bool doPhotonDynamicIsolation)
{

   m_applyDynamicIsolation = doPhotonDynamicIsolation;
}
...
int VJetsUncertaintyTool::initialize()
{
   ...
   //   std::vector<TString> processes = {"vvj"};
   ...
   for (unsigned int iP = 0; iP < processes.size(); iP++) {
      ...
      TString suffix = "";
      if (process == "aj") {
	if (m_applyDynamicIsolation) suffix = "_dyn_iso";
	else suffix = "_fix_iso";
      }
	 ...
	 for (Int_t p = 0; p < 8+m_nPDF; p++)  {
	   e[p] = 0;
         ...
         for (int bin = 1; bin <= hMC_used->GetNbinsX() + 1; bin++) {
            ...
            double dK_NNLO_fix = fabs(K_NLO_fix - K_NLO);
            ...
            double e_K_NNLO = K_NNLO + e[3] * d1K_NNLO + e[4] * d2K_NNLO + e[5] * d3K_NNLO + e[7] * dK_NNLO_fix;
...
const std::vector<TString> &VJetsUncertaintyTool::getAllVariationNames()
{
  ...
  if (!m_initialized) {
    std::cout << "ERROR: VJetsUncertaintyTool not initialized (called getAllVariationNames)" << std::endl;
...
double VJetsUncertaintyTool::getCorrection(int mcChannelNumber, double pTV, TString variation)
{
   ...
   TString process = "";
   // Single photon NLO
   if (364543 <= mcChannelNumber && mcChannelNumber <= 364547) process = "aj"; // gamma
   // Sherpa 2.2.1
   else if ((364142 <= mcChannelNumber && mcChannelNumber <= 364155)||mcChannelNumber==364222||mcChannelNumber==364223 ||
...
TH1 *rebinHisto(TH1 *h)
{
   ...
   const int nbins = 7;
   double limits[nbins + 1] = {30,70, 140, 280, 500, 1000, 2000, 6500};
