void plot_pdf_unc(TString folder1= "theoVariation_250319"){
  ...
  gSystem->Exec("mkdir -p /Users/othmanerifki/vbf/systematics/theoUnc_7bin/output/"+folder1+"/plots/pdf/");
  ...
  const int numfiles = 4; // 4 processes
  ...
  for (int ifile =0; ifile< numfiles; ifile++)
    for (int jvar =0; jvar< numvars; jvar++){
      ...
      TString file_inError = "/Users/othmanerifki/vbf/systematics/theoUnc_7bin/output/"+folder1+"/"+namefile+".root";
    ...
    uncStatOld[ifile][0] = err[0]/bin[0]; //cout << "Old: " << files[ifile] << " bin0: " << bin[0] << " +/- " << err[0] << " >> frac unc: " << uncStatOld[ifile][0] << endl;
    uncStatOld[ifile][1] = err[1]/bin[1]; //cout << files[ifile] << " bin1 " << uncStatOld[ifile][1] << endl;
    uncStatOld[ifile][2] = err[2]/bin[2]; //cout << files[ifile] << " bin2 " << unc[ifile][2] << endl;
...
// i=3 Wlnu CR
  ...
  TFile* fOut = new TFile("/Users/othmanerifki/vbf/systematics/theoUnc_7bin/output/"+folder1+"/plots/pdf/pdf_tf_old_new.root", "RECREATE");
  TList* hList = new TList();

// Z process
  TCanvas *c1 = new TCanvas( "PDF Z" , "PDF Z" );
...
//
  ...
  legend->Draw();
     ...
     // Write
  c1->Print( "/Users/othmanerifki/vbf/systematics/theoUnc_7bin/output/"+folder1+"/plots/pdf/pdf_tf_old_new_Z.pdf" );

// W process

  TCanvas *c2 = new TCanvas( "PDF W" , "PDF W" );
...
// Old
  ...
  for (int i=0; i<numbins; i++) {
    double A = uncSysOld_up[0][i];
    double dA = uncStatOld[0][i];
    double B = uncSysOld_up[1][i];
    double dB = uncStatOld[1][i];
    double tmp_tf = fabs((A)/(B)-1)*100.0;
    double tmp_tf_stat = Sqrt(Power(dA/B,2)+Power(A*dB/(B*B),2))*100;
...
// Old
  ...
  for (int i=0; i<numbins; i++) {
    ...
    double dA = uncStatOld[2][i];
    double B = uncSysOld_dn[3][i];
    double dB = uncStatOld[3][i];
    double tmp_tf = fabs((A)/(B)-1)*100.0;
    double tmp_tf_stat = Sqrt(Power(dA/B,2)+Power(A*dB/(B*B),2))*100;
...
//
  ...
  legendW->Draw();
     ...
     // Write
  c2->Print( "/Users/othmanerifki/vbf/systematics/theoUnc_7bin/output/"+folder1+"/plots/pdf/pdf_tf_old_new_W.pdf" );
