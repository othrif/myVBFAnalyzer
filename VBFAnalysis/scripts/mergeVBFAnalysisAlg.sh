...
export inputDir=$1
export outputDir=${2:-$inputDir}
export here=$PWD

echo "Openning root files from $inputDir"
cd ${inputDir}
export merged=${outputDir}"/merged"
echo "Writing merged files into $merged"
mkdir -p $merged
...
#hadd${merged}/ VV_VVV.root VV*root
