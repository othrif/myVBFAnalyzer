...
for syst in systlist:
    ...
    if "__1down" in syst or "Down" in syst:
        isLow = " --isLow"
    runCommand = '''athena VBFAnalysis/HFInputJobOptions.py --filesInput "$1" - --currentVariation '''+syst+isLow+' --currentSample "$1"'
    print runCommand
...
for syst in asys_systlist:
    runCommand = '''athena VBFAnalysis/HFInputJobOptions.py --filesInput "$1" - --currentVariation '''+syst+" --doLowNom"+' --currentSample "$1"'
    print runCommand
    writeCondorShell(workDir, buildDir, runCommand, syst, "HFInputCondorSub")
    writeCondorSub(workDir, syst, "HFInputCondorSub", listoffiles, listoffilesMC)
