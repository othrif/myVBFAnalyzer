...
def writeCondorShell(subDir, buildDir, runCommands, syst, scriptName="VBFAnalysisCondorSub", proxyName='/home/schae/testarea/HInv/run/x509up_u20186'):
    os.system('''echo "#!/bin/bash" > '''+subDir+'''/'''+scriptName+syst+'''.sh''')
    #os.system("echo 'export HOME="+subDir+"/rootFiles' >> "+subDir+"/"+scriptName+syst+".sh")
    os.system("echo 'export HOME=$(pwd)' >> "+subDir+"/"+scriptName+syst+".sh")
    os.system("echo 'cd "+subDir+"/rootFiles' >> "+subDir+"/"+scriptName+syst+".sh")
    ...
    for command in runCommands:
        os.system('''echo ' echo '''+command+'''' >> '''+subDir+'''/'''+scriptName+syst+'''.sh''')
        os.system('''echo ' '''+command+'''' >> '''+subDir+'''/'''+scriptName+syst+'''.sh''')
...
def writeCondorSub(workDir, syst="Nominal", scriptName="VBFAnalysisCondorSub", fileForArguments="filelist",fileForArgumentsSys=""):
    ...
    if syst!="Nominal": os.system("echo '+RequestRuntime     =  86400' >> "+workDir+"/submit_this_python"+syst+".sh")
